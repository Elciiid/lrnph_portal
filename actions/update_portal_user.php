...
$checkQuery = "SELECT COUNT(*) as count FROM LRNPH.dbo.lrnph_users WHERE (username = ? OR empcode = ?) AND user_id != ?";
...
if ($checkStmt) {
    ...
    if ($row['count'] > 0) {
        ...
        $specificCheckQuery = "SELECT (SELECT COUNT(*) FROM LRNPH.dbo.lrnph_users WHERE username = ? AND user_id != ?) as user_exists,
                                     (SELECT COUNT(*) FROM LRNPH.dbo.lrnph_users WHERE empcode = ? AND user_id != ?) as code_exists";
        $specStmt = sqlsrv_query($conn, $specificCheckQuery, array($username, $userId, $empcode, $userId));
...
if (!empty($password)) {
    ...
    $query = "UPDATE LRNPH.dbo.lrnph_users 
