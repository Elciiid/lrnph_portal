...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    try {
        ...
        $sql = "INSERT INTO \"prtl_AP_Meetings\" 
                ...
                VALUES (?, ?, ?, ?, ?::date, ?::time, ?::time, ?, ?, CURRENT_TIMESTAMP) 
        ...
        if (!$row) {
            throw new Exception("Failed to insert meeting");
        }
        $newMeetingId = $row['meeting_id'];
        ...
        if (!empty($agendas)) {
            ...
            foreach ($agendas as $agendaText) {
                if (!empty(trim($agendaText))) {
                    $agendaStmt->execute(array($newMeetingId, trim($agendaText)));
                }
            }
        }
        ...
        $attendeeInsertStmt = $conn->prepare($attendeeSql);
        ...
        if (isset($_POST['attendees']) && is_array($_POST['attendees'])) {
            ...
            foreach ($uniqueAttendees as $empId) {
                ...
                if ($detStmt && $emp = $detStmt->fetch(PDO::FETCH_ASSOC)) {
                    ...
                    $attendeeInsertStmt->execute(array($newMeetingId, $empId, $name, $dept));
        ...
        if (isset($_POST['custom_attendees']) && is_array($_POST['custom_attendees'])) {
            foreach ($_POST['custom_attendees'] as $customName) {
                ...
                if (!empty($customName)) {
                    // Insert with NULL ID
                    $attendeeInsertStmt->execute(array($newMeetingId, null, $customName, 'External Guest'));
                }
            }
        }
